...
| `GET /vcs/status` | workspace/move dialogs, all `.catch(() => undefined)` (`dialog-workspace-create.tsx:170`, `dialog-move-session.tsx:236`, `prompt/move.tsx:119`) | `200: Array<VcsFileStatus>` (`{ file: string; additions: number; deletions: number; status: "added"|"deleted"|"modified" }`, types.gen.ts:2311-2317) | `[]` | Skips "uncommitted changes" warnings |
...
- Everything under `/experimental/workspace` (create/remove/warp), `/experimental/project/*`, `/api/session/.../projectCopy`, `/experimental/control-plane/move-session`, provider OAuth (`/provider/{id}/oauth/*`), `PUT /auth/{providerID}`, `POST /global/upgrade` (app.tsx:1058): user-initiated, error-toast handled. A JSON 404 (§5.5) is acceptable; do not stub `true` for flows you can't actually perform (e.g., workspace create), or the UI will proceed on a lie.
