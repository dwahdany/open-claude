...
| `AskUserQuestion` | `{questions: [...]}` (SDKTOOLS:798) | `question` | see 04-permissions doc for the Question flow. Shim extras: option `preview` folds into the TUI-visible `description` (quote-barred `│ ` lines, 24-line budget per question split across preview options — the question dialog bottom-clips past terminal height with no scroll; raw `preview` also rides on the wire option, ignored by the TUI, no runtime validation client-side). A synthetic trailing "Notes" question (header `Notes`, option `No note`, free text via the built-in custom row) is appended unless `OPENCLAUDE_NO_QUESTION_NOTES=1`; on reply it is stripped, the note returns via `updatedInput.annotations[firstQuestion].notes` (the CLI renders ` notes: <text>` after that question's answer in the tool_result — probe-verified in test/probe-question-annotations.ts, which also verified: non-label answer strings pass verbatim, annotations bind per question, structured echo in `tool_use_result`), and `metadata.answers` stays aligned to the model's original questions with `note: <text>` appended to the first answer array (the TUI transcript joins extras with ", ") |
