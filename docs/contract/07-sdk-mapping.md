...
| `AskUserQuestion` | `{questions: [...]}` (SDKTOOLS:798) | `question` | see 04-permissions doc for the Question flow. Shim extras: option `preview`s render as a synthetic transcript TEXT part on the current assistant message right before `question.asked` (fenced markdown block per option, fence sized past any backtick runs inside the preview, `header · label` titles on multi-question dialogs) — the transcript is scrollable so previews are unbounded, whereas the question dialog bottom-clips past terminal height with no scroll; the raw `preview` also rides on the wire option (ignored by the TUI, no runtime validation client-side). Notes use the dialog's own free-text row with a delimiter (engine `splitNotes`): a typed entry `answer // note` splits into answer + note, an entry that is just `// note` is a pure note (multi-select: rides alongside toggled labels); both forms require the space-padded delimiter (so `//cdn.example.com` stays an answer), and entries exactly matching a picked option label bypass parsing entirely (model-authored labels may contain ` // `); notes return via `updatedInput.annotations[question].notes` PER QUESTION (the CLI renders ` notes: <text>` after each question's answer in the tool_result — probe-verified in test/probe-question-annotations.ts, which also verified: non-label answer strings pass verbatim, annotations bind per question, structured echo in `tool_use_result`), and `metadata.answers` carries `note: <text>` appended to that question's answer array (the TUI transcript joins extras with ", "). Loose reply shapes (bare string per answer) are normalized — a throw after `pendingQuestions.delete` would strand `canUseTool` and hang the turn |
